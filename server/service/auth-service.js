...
class AuthService {
  ...
  async login(login, password) {
    ...
    if (!user) {
      return { message: '[INFO] User not found!', flag: '#D1557A', code: 400 }
    }
  ...
  async register(email, password, login, role) {
    try {
      ...
      await user.save()
      return { message: '[INFO] User created!', flag: '#66D9BD', code: 200 }
    } catch (e) {
      console.log(e)
  ...
  async forgot(email) {
    try {
      ...
      if (!UserDb) {
        return {
          message: '[INFO] User with this email not found!',
          flag: '#EAC15A',
          code: 400,
        }
      }
